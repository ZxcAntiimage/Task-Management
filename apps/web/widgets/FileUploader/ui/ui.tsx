    ...
    return (
        <div className="w-full space-y-4">
            ...
            >
                ...
                <p className="text-sm font-medium text-[#141522]">
                    {isDragActive ? t("fileUploader.dropFiles") : t("fileUploader.dragFiles")}
                </p>
                <p className="text-xs text-[#54577A] mt-1">{t("fileUploader.acceptedFormats")}</p>
            </div>

            {files.length > 0 && (
                <ul className="space-y-3 max-h-52 overflow-y-auto pr-1">
                    {files.map((file) => (
                        <li key={file.id} className="p-3 bg-secondary/20 rounded-xl border space-y-2">
                            <div className="flex items-center justify-between gap-2 text-sm">
                                <div className="flex items-center gap-2 truncate flex-1">
                                    <File className="h-4 w-4 text-muted-foreground shrink-0" />
                                    <span className="truncate text-[#141522] font-medium">{file.name}</span>
                                    <span className="text-xs text-[#54577A] shrink-0">
