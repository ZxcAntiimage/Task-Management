...
export default function Mentors() {
    ...
    return (
        <div className="w-full min-h-screen m-0 bg-[#FAFAFA] flex flex-col gap-6 sm:gap-10 overflow-y-auto">
            <header className="p-4 sm:p-8 bg-white w-full border-b flex flex-col border-[#F0F0F0]">
                <div className="flex flex-row justify-between items-center flex-wrap gap-4">
                    <h1 className="text-xl sm:text-2xl text-[#141522] font-semibold leading-[150%] tracking-[-3%]">{t("mentors.exploreMentors")}</h1>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mt-6">
                    <ButtonGroup className="relative flex flex-col items-center w-full sm:w-120 h-13">
                        <div className="flex flex-row items-center w-full">
                            <Input value={query} onChange={handleSearchChange} className="text-[#54577A] text-[12px] w-full h-13 px-7 pr-12 py-3.5 border-[#54577A] border rounded-[10px]" id="input-button-group" placeholder={t("mentors.searchMentors")} />
            ...
            <div className="p-4 sm:p-10">
                <section className="flex flex-col w-full mb-8">
                    <header className="flex flex-row items-center justify-between mb-6 flex-wrap gap-4">
                        <span className="text-[#141522] text-xl sm:text-2xl font-bold tracking-[-2%]">{t("mentors.recentMentors")}</span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 w-full">
                        {recentVisible.map((mentor) => {
                            ...
                            return (
                                <Card key={`recent-${mentor.id}`} className="w-full bg-white rounded-[20px] p-4 sm:p-6 flex flex-col gap-5 border-none shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
                                    ...
                                    <CardFooter className="flex flex-row items-center justify-between p-0 pb-6 border-t bg-white border-[#F5F5F7] pt-4">
                                        <div className="flex flex-row items-center gap-2">
                                            ...
                                            <span className="text-[#141522] text-xs sm:text-sm font-semibold tracking-[-2%]">{mentor.totalTask} {t("mentors.taskCount")}</span>
                                        </div>
                ...
                <section className="flex flex-col w-full">
                    <header className="flex flex-row items-center justify-between mb-6 flex-wrap gap-4">
                        <span className="text-[#141522] text-xl sm:text-2xl font-bold tracking-[-2%]">{t("mentors.mentors")}</span>
                        <Pagination className="w-auto m-0">
                            <PaginationContent className="flex flex-row gap-2">
                                <PaginationItem>
                                    <Button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 w-full">
                        {allVisible.map((mentor) => {
                            const isFollowed = followedIds.includes(mentor.id);
                            return (
                                <Card key={`all-${mentor.id}`} className="w-full bg-white rounded-[20px] p-4 sm:p-6 flex flex-col gap-4 border-none shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
                                    <div className="flex flex-row items-center justify-between w-full">
                                        <CardContent className="flex flex-row items-center gap-3 p-0">
                                            <Image src={mentor.avatar || ""} alt={mentor.name} loading="lazy" width={48} height={48} className="rounded-full object-cover shrink-0 w-10 h-10 sm:w-12 sm:h-12" />
                                            <div className="flex flex-col">
                                                <span className="text-[#141522] text-sm sm:text-base font-bold tracking-[-2%]">{mentor.name}</span>
                                                <span className="text-[#8E92BC] text-[10px] sm:text-[12px] font-medium tracking-[-1%]">{mentor.position}</span>
                                            </div>
                                        </CardContent>
                                        <CardAction onClick={() => handleFollowToggle(mentor.id)} className="cursor-pointer p-0 select-none shrink-0">
                                            <span className={`text-xs sm:text-sm font-semibold tracking-[-2%] ${isFollowed ? "text-[#54577A]" : "text-[#546FFF]"}`}>
                                    ...
                                    <p className="text-[#8E92BC] text-[10px] sm:text-[12px] font-normal leading-[160%] tracking-[-1%] min-h-10 line-clamp-2">
